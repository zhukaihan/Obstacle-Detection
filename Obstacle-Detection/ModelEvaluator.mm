...
@interface ModelEvaluator (InternalMethods)
@end

@implementation ModelEvaluator {
...
- (NSArray*)evaluateOnBuffer:(CMSampleBufferRef)sampleBuffer {
  ...
  NSArray* labeledArr = [self runModelOnFrame:pixelBuffer];
  CFRelease(pixelBuffer);
  return labeledArr;
}

- (NSArray*)runModelOnFrame:(CVPixelBufferRef)pixelBuffer {
  ...
  if (is_quantized) {
    ...
    float* output = interpreter->typed_output_tensor<float>(2);
    GetTopN(output, output_size, kNumResults, kThreshold, &top_results);
  }
    ...
    for (const auto& result : top_results) {
        ...
        NSLog(@"\nbox%d is at: %d, %d, %d, %d", index, output_boxes[index * 4], output_boxes[index * 4 + 1], output_boxes[index * 4 + 2], output_boxes[index * 4 + 3]);
    }
